...
export default function NotificationsScreen() {
  ...
  const [loadingFalls, setLoadingFalls] = useState(true);
  ...
  const loadFalls = useCallback(async () => {
    if (!pacienteId) {
      setFalls([]);
      setLoadingFalls(false);
      return;
    }

    try {
      setLoadingFalls(true);
      const response = await apiService.get(`/api/pacientes/${pacienteId}/caidas`);
      console.log("[AlertasScreen] Caídas response:", JSON.stringify(response));

      const caidasData = response.caidas ?? [];

      const caidasFormateadas: FallEvent[] = caidasData.map((item: any, index: number) => {
        // El timestamp viene como string ISO "2026-07-20T23:30:00"
        const fechaObj = item.timestamp ? new Date(item.timestamp) : new Date();

        return {
          id: String(item.id || index),
          time: fechaObj.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true }),
          date: fechaObj.toLocaleDateString(),
          probabilidad: item.probabilidad ?? 0
        };
      });

      setFalls(caidasFormateadas);
    } catch (error) {
      console.error("[NotificationsScreen] Error cargando historial de caídas:", error);
      setFalls([]);
    } finally {
      setLoadingFalls(false);
    }
  ...
  }, [pacienteId, actualizarMedicamentos]);
  ...
  return (
    <PaperProvider theme={theme}>
      ...
      <View style={globalStyles.container}>
        {/* Se eliminó el <View style={globalStyles.content}> superior duplicado */}
        ...
        <FlatList
          ...
          contentContainerStyle={[globalStyles.content, { paddingBottom: 80 }]}
          refreshing={loadingFalls}
          onRefresh={loadFalls}
          ListHeaderComponent={() => (
            <View>
              <Text variant="headlineSmall" style={globalStyles.title}>
                {t('alerts.title')}
              </Text>

              {/* Recordatorio de Medicamentos */}
              <Text variant="titleMedium" style={styles.sectionTitle}>
                {t('alerts.medication_reminder')}
              </Text>
              ...
              {/* Título de la sección de Caídas */}
              <Text variant="titleMedium" style={styles.sectionTitle}>
                {t('alerts.falls_detected')}
              </Text>
            </View>
          )}
          renderItem={({ item }) => (
        ...
        <Portal>
          <Modal
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              <Text style={[globalStyles.modalTitle, { marginBottom: 10 }]}>
                {medicamentoEditado ? t('alerts.edit_medication') : t('alerts.add_medication')}
              </Text>

              <Text style={[globalStyles.inputLabel, { marginBottom: 10 }]}>
              ...
              <Button mode="outlined" onPress={abrirSelectorHora} style={{ marginBottom: 10 }}>
